	...
	<script type="text/javascript">
	 function queryAllUser(pageNum, pageSize) {
		<%--  $(".table-responsive").load("<%=contextPath %>/listUser.action?pageNum=" + pageNum + "&pageSize=" + pageSize); --%>
	        window.location.href="<%=contextPath %>/listAdmin.action?pageNum=" + pageNum + "&pageSize="+ pageSize;
	    }
	</script>
</head>
			...
			<div class="table-responsive">
				<table class="table table-striped table-bordered table-hover no-border-top">
					...
					<c:forEach items="${pagehelper.list}" var="admin">
						 <tr>
							<td><a href="">${admin.admusername}</a></td>
							
							<td><a href="">${admin.admname}</a></td>
		
						<%-- 	<c:choose>
					           <c:when test="${admin.type==1 }">
					              <td><a href="">超级管理员</a></td>
					            </c:when>
					            <c:otherwise>
						            <td><a href="">普通管理员</a></td>
						        </c:otherwise>
					        </c:choose> --%>
							...
							<td>
								<div class="action-buttons">
									<a class="blue" href="toModifyAdmin.action?admid=${admin.admid}">
										<i class="icon-pencil blue"></i>修改
									</a>
									<a class="blue" href="javascript:void(0)"  onclick="delAdmin('${admin.admid}')">
	...
	<script>
		$(function(){
			$("#chuangjian").click(function(){
				window.location.href="toAddAdmin.action";
			});
		});
		function delAdmin(admid){
		    layer.confirm('确认删除？', {
			  ...
			  window.location.href="delAdmin.action?admid="+admid;
			});
		}
	</script>
</body>
